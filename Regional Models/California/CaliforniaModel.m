...
load('Calidata.txt')
years = Calidata(:,1); % years
qinj =  Calidata(:,2); % MT - storage rate 
Q =  Calidata(:,3)./1000; % Gt - cumulative storage 
...
% rate change
rtarget = [0.18167, 0.18264, 0.1953];
% resulting storage to meet 2100 target
Qtarget =[85.2084, 30.2950, 2.9862];
% resulting peak injection
peak_target = [2078.9 2073, 2058.3];
...
for i=1:length(Qtarget)
    ...
    cum_at_rate_change = exp(year_rate_change.*w).*exp(-81.779792);
...
for i=1:length(Qtarget)
    cum_2030 = exp(year_rate_change.*w).*exp(-81.779792);
    x2 = [year_rate_change:2150];
...
plot(2050, 0.08, '.r', 'markersize', 12,'HandleVisibility','off')
...
text(2120,3.45, 'California', 'fontsize', 12, 'FontWeight', 'bold')
%% Now plot target tradeoff curves
subplot(1,3,3)
hold on
axis_def = [15 30 0.5 1000];
...
text(15.3, 0.9, sprintf('Storage rate by 2050'), 'fontsize', 14)
