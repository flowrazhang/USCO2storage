...
% rate change
rtargetMD = [0.06857, 0.07032];% growth rate modelled 
rtargetND = [0.043519, 0.04618];
rtargetCa = [0.18167, 0.18264, 0.1953];
rtargetGC = [0.13795, 0.13862  0.151];
% resulting storage to meet 2100 target
QtargetMD = [153.3015, 12.0857];% storage resource modelled 
QtargetND = [141.4287, 15.0070];
QtargetCa = [85.2084, 30.2950, 2.9862];
QtargetGC =[1753.1, 366.9790  36.3404];
...
load('target47.txt')
Rr = target47(:,1);  % grpwth rate
M = target47(:,2); % Qmin
plot(Rr.*100, M, 'k', 'color', greycc(end,:))
hold on
load('target33.txt')
Rr = target33(:,1);  % grpwth rate
M = target33(:,2); % Qmin
plot(Rr.*100, M, '-', 'color', greycc(end-3,:))
hold on
load('target769.txt')
Rr = target769(:,1);  % grpwth rate
M = target769(:,2); % Qmin
plot(Rr.*100, M, '-', 'color', greycc(end-4,:))
hold on
load('target80.txt')
Rr = target80(:,1);  % grpwth rate
M = target80(:,2); % Qmin
...
for i=1:length(QtargetGC)
    cum_2030 = exp(year_rate_change.*0.152439003).*exp(-310.4750519);
...
legend({['47 Mt/yr, North Dakota Hub'],['33 Mt/yr, Mideastern Hub'],['769 Mt/yr, Gulf Coast Hub'],['80 Mt/yr, California Hub'],}, ...
