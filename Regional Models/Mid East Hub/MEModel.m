...
load('MDHub.txt')
years = MDHub(:,1); % years
qinj =  MDHub(:,2); % MT - storage rate 
Q =  MDHub(:,3)./1000; % Gt - cumulative storage 
...
% rate change
rtarget = [0.06857, 0.07032];
% resulting storage to meet 2100 target
Qtarget =[153.3015, 12.0857];
% resulting peak injection
peak_target = [2134.1 2095.1];
...
for i=1:length(Qtarget)
    ...
    cum_at_rate_change = exp(year_rate_change.*w).*exp(-646.6528894)
...
for i=1:length(Qtarget)
    cum_2030 = exp(year_rate_change.*w).*exp(-646.6528894);
    x2 = [year_rate_change:2150];
...
plot(2050, 0.0326, '.r', 'markersize', 12,'HandleVisibility','off')
...
text(2100,3.45, 'Mideastern Hub', 'fontsize', 12, 'FontWeight', 'bold')
...
legend({['33 Mt/yr, Mideastern Hub']}, ...
