...
% rate change
rtarget = [0.043519, 0.04618];
% resulting storage to meet 2100 target
Qtarget =[141.4287, 15.0070];
% resulting peak injection
peak_target = [2161.7 2104.4];
...
for i=1:length(Qtarget)
    ...
    cum_at_rate_change = exp(year_rate_change.*w).*exp(-52.47182071)
...
for i=1:length(Qtarget)
    cum_2030 = exp(year_rate_change.*w).*exp(-52.47182071);
    x2 = [year_rate_change:2150];
...
plot(2050, 0.0467, '.r', 'markersize', 12,'HandleVisibility','off')
...
legend({['46.7 Mt/yr, North Dakota Hub']}, ...
